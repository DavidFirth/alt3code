webify2 <- function(league, season) {
...
#    ht$sched <- gsub("-", "minus", ht$sched)
    ...
    text_color(ht)[c(FALSE, very_tough_schedule), 9] <- "darkgreen"
    text_color(ht)[c(FALSE, very_easy_schedule), 9] <- "darkred"
    ...
    background_color(ht)[3:22, 3:6] <- "gainsboro"
    text_color(ht)[3:22, 3:6] <- "#666666"
...
#    row_height(ht) <- 9
    ...
    ht <- set_bottom_border_color(ht, c(3:21), 3:6, "#cccccc")
    ht <- set_bottom_border_color(ht, c(3:21), c(1:2, 8:10), "#ffa74d")
