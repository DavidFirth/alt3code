...
fetch_league_table <- function(league, season, matchday = NULL, source = "BBC"){
    ...
    if (source == "fdo") {
        ...
        outfile <- paste(league, "/", season, "/",
                         if (is.null(matchday)) "leagueTable.json"
                         else matchday_filename,
                         sep = "")
    ...
    if (source == "BBC") {
        BBCname <- leagues[league, "BBC_name"]
        outfile <- paste(league, "/", season, "/",
                         "leagueTable.html",
                         sep = "")
        url <- paste0("https://www.bbc.co.uk/sport/football/",
                      BBCname,
                      "/table")
        r <- system2("curl", url, stdout = outfile)
        if (r == 0) cat("The data are in: ", outfile, "\n")
        else cat("There was an error!\n")
        invisible(r)
    }
